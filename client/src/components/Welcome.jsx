...
function Welcome() {
  // Scroll animation effect for staggered entrance
  useEffect(() => {
    ...
    elements.forEach((el, i) => {
      el.style.transitionDelay = `${i * 200}ms`; // stagger by 0.2s
      observer.observe(el);
    });
  }, []);

  return (
    <section className="bg-green-50 w-full min-h-screen flex flex-col">
      {/* Header */}
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center flex-1 px-6 lg:px-20 gap-10 mt-10">
        ...
        <div className="text-center lg:text-left max-w-lg fade-in-stagger opacity-0 translate-y-10">
          ...
          <div className="flex justify-center lg:justify-start gap-4 fade-in-stagger opacity-0 translate-y-10">
            <Link
              to="/register"
              className="bg-green-400 text-white font-semibold px-6 py-3 rounded-full shadow-md hover:bg-green-500 transition duration-500 transform hover:scale-105"
            >
              Get Started
            </Link>
            <Link
              to="/login"
              className="border border-green-400 text-green-600 font-medium px-5 py-3 rounded-full hover:bg-green-50 transition duration-500 transform hover:scale-105"
            >
              Login
            </Link>
          </div>
        ...
        <div className="flex justify-center items-center mt-4 max-w-lg  fade-in-stagger opacity-0 translate-y-10">
          <img
            src={welcome}
            alt="Welcome"
            className="rounded-lg shadow-xl w-full max-w-md transform p-6 transition duration-700 hover:scale-105"
          />
        </div>
      </div>
    </section>
